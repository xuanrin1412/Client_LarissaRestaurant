...
function MenuOrderTable() {
    ...
    const filteredCategoryWFood = categoryWFood.map(category => ({
        ...category,
        food: category.food.filter(foodItem =>
            foodItem.foodName.toLowerCase().includes(textSearch.toLowerCase())
        )
    }));
    ...
    return (
        <div className="my-header mx-4 lg:mx-10">
            <div className="pt-5">
                <div className=" border border-black rounded-lg w-10/11 lg:w-2/3 mx-auto flex items-center justify-center">
                    <input
                        value={textSearch}
            ...
            {filteredCategoryWFood?.map((category, index) => (
                <div key={index}>
                            ...
                            <div className={`grid ${menuActive ? "lg:grid-cols-4" : "lg:grid-cols-5"} gap-4 lg:gap-6`}>
                                {category.food.map((food, index) => (
                                    <div
                                        ...
                                        key={index}
                                        className="w-full flex flex-col h-fit bg-primary text-white rounded-lg shadow"
                                    >
